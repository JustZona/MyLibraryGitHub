package com.example.mylibrary.control.Image;
...
public class PinchImageView extends ImageView  {
     ...
     * 2:图片充满控件高度.
     * 3:图片高宽为控件高宽.
     * 4:图片高度为控件高度，宽度按照比例放大、缩小.
     * 5:图片宽度为控件宽度，高度按照比例放大、缩小.
     */
    public int initModel = 5;
     ...
     * 图片最大放大比例
     */
    public static float MAX_SCALE = 4f;

    /**
     * 图片最大缩小比例
     */
    public static float Mix_SCALE = 0.5f;
        ...
        if (isReady()) {
            ...
            switch (initModel){
                ...
                case 1:
                    ...
                    matrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.FILL);
                    MathUtils.rectFGiven(tempSrc);
                    MathUtils.rectFGiven(tempDst);
                    break;
                case 2:
                    tempSrc = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
                    tempDst = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getHeight());
                    matrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.FILL);
                    MathUtils.rectFGiven(tempSrc);
                    MathUtils.rectFGiven(tempDst);
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    float j = (float) (getHeight()*1.0/getDrawable().getIntrinsicHeight());
                    float width = j*getDrawable().getIntrinsicWidth();
                    tempSrc = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
                    tempDst = MathUtils.rectFTake(point.x/2-width/2, 0, width+point.x/2-width/2, getHeight());
                    matrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.FILL);
                    MathUtils.rectFGiven(tempSrc);
                    MathUtils.rectFGiven(tempDst);
                    break;
                case 5:
                    float i = (float) (getWidth()*1.0/getDrawable().getIntrinsicWidth());
                    tempSrc = MathUtils.rectFTake(0, 0, getDrawable().getIntrinsicWidth(), getDrawable().getIntrinsicHeight());
                    tempDst = MathUtils.rectFTake(0, 0, getWidth(), i*getDrawable().getIntrinsicHeight());
                    matrix.setRectToRect(tempSrc, tempDst, Matrix.ScaleToFit.FILL);
                    MathUtils.rectFGiven(tempSrc);
                    MathUtils.rectFGiven(tempDst);
                    break;
    ...
    protected float calculateNextScale(float innerScale, float outerScale) {
        ...
        if (currentScale < getMaxScale()&&currentScale>=initScale) {
            return getMaxScale();
        } else if(currentScale>getMixScale()&&currentScale<1f){
            return getMixScale();
        }else {
            return innerScale;
        }
     ...
     * @see #doubleTap(float, float)
     */
    protected float getMixScale() {
        return Mix_SCALE*initScale;
    }
    ...
    protected void onDraw(Canvas canvas) {
        //在绘制前设置变换矩阵
        if (point==null){
            point = AdaptationUtil.getDisplay(getContext());
        }
        if (isReady()) {
            Matrix matrix = MathUtils.matrixTake();
            ...
            if(isFirst){
                isFirst = false;
                initScale = MathUtils.getMatrixScale(matrix)[0];
            }
            MathUtils.matrixGiven(matrix);
        }
        //对图像做遮罩处理
    ...
    private void scaleEnd() {
        ...
        if (outerScale * scalePost < mixScale) {
            scalePost = mixScale / outerScale;
        }
        //如果缩放修正不为1，说明进行了修正
        if (scalePost != 1f) {
            change = true;
