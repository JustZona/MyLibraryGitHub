package com.example.mylibrary.control.Refresh.ptrlib;
...
import com.example.mylibrary.control.Refresh.ptrlib.util.PtrCLog;
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (DEBUG) {
                    ...
                    PtrCLog.v(LOG_TAG, "ACTION_MOVE: offsetY:%s, currentPos: %s, canMoveUp: %s, moveDown: %s: canMoveDown: %s", offsetY, mPtrIndicator.getCurrentPosY(), canMoveUp, moveDown, canMoveDown);
                }
                if (moveDown && mPtrHandler != null && !mPtrHandler.checkCanDoRefresh(this, mContent, mHeaderView)) {
                    return dispatchTouchEventSupper(e);
                }else {
                    if (canMoveUp || moveDown) {
//                        Log.v("moveDown",""+moveDown);
//                        Log.v("canMoveUp",""+canMoveUp);
                        movePos(offsetY);
                        return true;
                    }
                }
    ...
    protected void onPtrScrollAbort() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                PtrCLog.d(LOG_TAG, "call onRelease after scrollFY abort");
            }
            onRelease(true);
        }
    }
    ...
    protected void onPtrScrollFinish() {
        if (mPtrIndicator.hasLeftStartPosition() && isAutoRefresh()) {
            if (DEBUG) {
                PtrCLog.d(LOG_TAG, "call onRelease after scrollFY finish");
            }
            onRelease(true);
        }
    }
    ...
    class ScrollChecker implements Runnable {
        ...
        public void run() {
            ...
            Log.v("curY",curY+"");
            Log.v("deltaY",deltaY+"");
            Log.v("mLastFlingY",mLastFlingY+"");
            if (DEBUG) {
                if (deltaY != 0) {
                    PtrCLog.v(LOG_TAG,
                            "scrollFY: %s, start: %s, to: %s, currentPos: %s, current :%s, last: %s, delta: %s",
